...
void Note::endDrag()
      ...
      if (staff->isTabStaff()) {
            ...
            if (altNotation && altNotation->octaveDistance() != 7) { //cc octave check only temporary until proper
                  int octaveDistance = altNotation->octaveDistance(); //   key-awareness is implemented
                  int correction = 5 * (octaveDistance - 7);          //   note: This may cause incorrect note placement if a 7-distance octave is not traditionally mapped.
