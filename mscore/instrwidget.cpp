...
void StaffListItem::setStaffType(const StaffType* st)
      ...
      if (!st)                                        // if no staff type given, dault to stadard
            ...
            for (int i = 0; i < _staffTypeCombo->count(); ++i) {
                  const StaffType* _st = StaffType::preset(_staffTypeCombo->itemData(i).toInt()); //cc TODO: CONFIRM
                  if (*_st == *st) {
...
const StaffType* StaffListItem::staffType() const
      {
      return StaffType::preset((staffTypeIdx())); //cc TODO: CONFIRM
      }
...
void StaffListItem::staffTypeChanged(int idx)
      ...
      const StaffType* stfType = StaffType::preset(staffTypeIdx); //cc TODO: CONFIRM
...
void PartListItem::updateClefs()
      ...
      for (int i = 0; i < childCount(); ++i) {
            ...
            const StaffType* stfType = StaffType::preset(sli->staffTypeIdx()); //cc TODO: CONFIRM
